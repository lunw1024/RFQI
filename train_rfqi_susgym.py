...
def eval_policy(policy, env_name, eval_episodes=10):
    ...
    for _ in range(eval_episodes):
        state, info = env.reset(seed=np.random.randint(100000))
        done = False
        eps_reward = 0.0
        while not done:
...
if __name__ == "__main__":
    ...
    parser.add_argument('--env', default='BuildingEnv-v0', type=str)
    ...
    assert args.env == 'BuildingEnv-v0', "Env not suppoered"
